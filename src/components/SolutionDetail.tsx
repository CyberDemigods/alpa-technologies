...
/** Klientowa treść podstrony rozwiązania — strona serwerowa zostaje tylko dla generateStaticParams. */
export default function SolutionDetail({ slug }: SolutionDetailProps) {
  return (
    <Suspense>
      <SolutionDetailContent slug={slug} />
    </Suspense>
  );
}

function SolutionDetailContent({ slug }: SolutionDetailProps) {
  const lang = useLang();
